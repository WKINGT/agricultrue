...
public class MemberBlockDataService extends BaseService {
    ...
    public Map<String,Object> findIndexDataByMember(String memberId){
        ...
        List<ViewMachineBlockType> machineBlockTypes = ViewMachineBlockType.dao.find("SELECT type_name,COUNT(machine_id) as types,is_main FROM view_machine_block_type where block_id in ("+ StrUtils.joinInSql(blocks)+") GROUP BY type_id");
        String sql = "SELECT COUNT(CASE WHEN end_time<NOW() THEN 1 ELSE NULL END) as yes," +
                "COUNT(CASE WHEN end_time>NOW() THEN 1 ELSE NULL END) as no FROM base_task_plan_job WHERE block_id in ("+ StrUtils.joinInSql(blocks)+") and  status = ?  limit 1 ";
        Record task = Db.findFirst(sql, StatusEnum.NORMAL_USE.getValue());
