...
import net.xgs.commons.utils.IOUtils;
import net.xgs.commons.utils.URLUtils;
...
public abstract class FileSearcher {
	...
	private void doLookupInFileSystem(File dir, String pkgdir,
			String relativeName) {
		...
		for (File file : files) {
			String name = (relativeName == null) ? file.getName()
	...
	private void doLookupInVfsFile(URL url, String pkgdir) {
		try {
			...
			if (conn.getClass().getName()
					.equals("org.jboss.vfs.protocol.VirtualFileURLConnection")) {
		...
		public String getQualifiedJavaName() {
			...
			if (file.isDirectory()) {
				...
				throw new IllegalStateException(
						"FileEntry is not a Java Class: " + toString());
			}
		}
